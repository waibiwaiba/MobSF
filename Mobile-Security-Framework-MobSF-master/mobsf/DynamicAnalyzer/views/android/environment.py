...
class Environment:
    ...
    def check_connect_error(self, output):
        ...
        print(f"output:\n{output}")#LX
        if (
            b"adbd is already running as root" in output
            or b"restarting adbd as root" in output
        ):
            logger.info("%s", output.decode("utf-8").replace("\n", ""))
            print("\n1True\n")  #LX
            return True
        elif (
            b"unable to connect" in output
            or b"failed to connect" in output
            or b"cannot resolve host" in output
        ):
    ...
    def run_subprocess_verify_output(self, cmd, wait=2, timeout=None):
        """Run subprocess and verify execution."""
        # import pdb; pdb.set_trace()#DEBUG
        print(f"cmd:\n{cmd}")#LX
        out = subprocess.check_output(cmd, timeout=timeout)  # lgtm [py/command-line-injection] #LX 加一个timeout参数
    ...
    def connect(self):
        ...
        #LX useless
        # self.run_subprocess_verify_output(
        #     [
        #         get_adb(),
        #         #
        #         "connect",
    ...
    def connect_n_mount(self):
        ...
        if not self.identifier:
            return False
        # self.adb_command(["kill-server"]) #LX
        # self.adb_command(["start-server"], False, True)#LX
        ...
        #LX
        # 我认为这里的connect没什么用
        # if not self.run_subprocess_verify_output([get_adb(),"connect",
        #                                           # "-s",
        #                                           self.identifier,]):
        #     return False
        ...
        self.adb_command(["echo", "[OK]:Connect to AVD, Ready for root."], True, True)#LX
        if not self.run_subprocess_verify_output(
            [get_adb(), "-s", self.identifier, "root"], timeout=15
        ):
            return False
    ...
    def install_apk(self, apk_path, package, reinstall):
        ...
        # Install APK
        out = self.adb_command(["install", "-r", "-t", "-d", apk_path], False, True)
        if not out:
    ...
    def adb_command(self, cmd_list, shell=False, silent=False):
        ...
        try:
            ...
            result = subprocess.check_output(
                args, stderr=subprocess.STDOUT  # lgtm [py/command-line-injection]
            )
            return result
        except Exception:
            if not silent:
    ...
    def dz_cleanup(self, bin_hash):
        ...
        screen_dir = os.path.join(settings.UPLD_DIR, bin_hash + "/screenshots-apk/")
    ...
    def install_mobsf_ca(self, action):
        ...
        if is_file_exists(mobsf_ca):
            ...
            ca_file = os.path.join(
                "/system/etc/security/cacerts/", ca_construct.format(ca_file_hash)
            )
            pem.close()
        else:
    ...
    def set_global_proxy(self, version):
        ...
        if proxy_ip:
            if version < 4.4:
                logger.warning(
                    "Please set Android VM proxy as %s:%s", proxy_ip, proxy_port
                )
                return
    ...
    def unset_global_proxy(self):
        ...
        self.adb_command(["settings", "delete", "global", "http_proxy"], True)
        self.adb_command(
            ["settings", "delete", "global", "global_http_proxy_host"], True
        )
        self.adb_command(
            ["settings", "delete", "global", "global_http_proxy_port"], True
        )
        self.adb_command(["settings", "put", "global", "http_proxy", ":0"], True)
    ...
    def start_clipmon(self):
        ...
        args = ["am", "startservice", "opensecurity.clipdump/.ClipDumper"]
        self.adb_command(args, True)

    def screen_shot(self, outfile):
        """Take Screenshot."""
        self.adb_command(["screencap", "-p", "/data/local/screen.png"], True, True)
        self.adb_command(["pull", "/data/local/screen.png", outfile], False, True)

    def screen_stream(self):
        """Screen Stream."""
        self.adb_command(["screencap", "-p", "/data/local/stream.png"], True, True)
    ...
    def get_environment(self):
        ...
        out = self.adb_command(["getprop", "ro.boot.serialno"], True, False)
        out += self.adb_command(["getprop", "ro.serialno"], True, False)
        out += self.adb_command(["getprop", "ro.build.user"], True, False)
        out += self.adb_command(["getprop", "ro.manufacturer.geny-def"], True, False)
        out += self.adb_command(
            ["getprop", "ro.product.manufacturer.geny-def"], True, False
        )
        ver = self.adb_command(
            ["getprop", "ro.genymotion.version"], True, False
        ).decode("utf-8", "ignore")
        if b"EMULATOR" in out:
            logger.info("Found Android Studio Emulator")
            return "emulator"
        elif b"genymotion" in out.lower() or any(char.isdigit() for char in ver):
    ...
    def get_android_arch(self):
        ...
        out = self.adb_command(["getprop", "ro.product.cpu.abi"], True)
        return out.decode("utf-8").rstrip()

    def get_android_sdk(self):
        """Get Android API version."""
        out = self.adb_command(["getprop", "ro.build.version.sdk"], True)
        return out.decode("utf-8").strip()

    def get_device_packages(self):
        """Get all packages from device."""
        device_packages = {}
        out = self.adb_command(["pm", "list", "packages", "-f", "-3"], True, True)
        if not out:
        ...
        for pkg_str in out.decode("utf-8").rstrip().split():
            ...
            if pkg == "opensecurity.clipdump":
                ...
                continue
            out1 = self.adb_command(["md5sum", "-b", apk], True)
    ...
    def get_apk_packages(self, package):
        ...
        out = self.adb_command(["pm", "path", package], True)
        return out.decode("utf-8").strip()
    ...
    def get_apk_parent_directory(self, package):
        ...
        if "package:" in package_out[0] and package_out[0].endswith(".apk"):
    ...
    def system_check(self, runtime):
        ...
        try:
            try:
                ...
                if api:
                    ...
                    if int(api) > ANDROID_API_SUPPORTED:
                        logger.error(
                            "This API Level is not supported" " for Dynamic Analysis."
                        )
                        return False
            except Exception:
                pass
            ...
            proc = subprocess.Popen(
                [get_adb(), "-s", self.identifier, "shell", "touch", "/system/test"],
                stdout=subprocess.PIPE,
                stderr=subprocess.PIPE,
            )
            _, stderr = proc.communicate()
            if b"Read-only" in stderr:
                ...
                if runtime == "emulator":
                    logger.error("Please start the AVD as per " "MobSF documentation!")
                return False
        except Exception:
            logger.error(err_msg)
            return False
    ...
    def launch_n_capture(self, package, activity, outfile):
        ...
        self.adb_command(["am", "start", "-n", package + "/" + activity], True)
    ...
    def xposed_setup(self, android_version):
        ...
        clip_dump_apk = os.path.join(self.tools_dir, xposed_dir, "ClipDump.apk")
        ...
        if android_version < 5:
            ...
            xposed_apk = os.path.join(self.tools_dir, xposed_dir, "Xposed.apk")
            hooks = os.path.join(self.tools_dir, xposed_modules, "hooks.json")
            droidmon = os.path.join(self.tools_dir, xposed_modules, "Droidmon.apk")
            ...
            xposed_apk = os.path.join(
                self.tools_dir, xposed_dir, "XposedInstaller_3.1.5.apk"
            )
        self.adb_command(["install", "-r", xposed_apk])
        # Xposed Modules and Support Files
        justrustme = os.path.join(self.tools_dir, xposed_modules, "JustTrustMe.apk")
        rootcloak = os.path.join(
            self.tools_dir, xposed_modules, "com.devadvance.rootcloak2_v18_c43b61.apk"
        )
        proxyon = os.path.join(
            self.tools_dir, xposed_modules, "mobi.acpm.proxyon_v1_419b04.apk"
        )
        sslunpin = os.path.join(
            self.tools_dir, xposed_modules, "mobi.acpm.sslunpinning_v2_37f44f.apk"
        )
        bluepill = os.path.join(self.tools_dir, xposed_modules, "AndroidBluePill.apk")
        ...
        self.adb_command(["am", "start", "-n", xposed_installer], True)
    ...
    def frida_setup(self):
        ...
        if not stat:
            msg = (
                "Cannot download frida-server binary. You will need"
        ...
        #LX
        hooked_api_list_win = r"F:\MobSFCODE\Mobile-Security-Framework-MobSF-master\mobsf\DynamicAnalyzer\tools\frida_scripts\android\default\hooked_api_list.csv"
        logger.info("Copying hooked api list file to AVD")
        self.adb_command(["push", hooked_api_list_win, "/data/local/tmp/hooked_api_list.csv"])
        self.adb_command(["chmod", "644", "/data/local/tmp/hooked_api_list.csv"], True)
    ...
    def run_frida_server(self):
        ...
        def start_frida():
            ...
            argz = [get_adb(), "-s", self.identifier, "shell", "/system/fd_server"]
